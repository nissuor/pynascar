...
## Driver aggregation notes

`DriversData.build()` processes finished races from oldest to newest so each
`Driver`'s name, team, car number, and manufacturer reflect its latest nonblank
results. Individual race records retain the information available at that race.
`race_ids` and `all_races_dataframe()` retain the schedule's newest-first race
order; the season summary's driver rows follow first appearance during the
oldest-first traversal. Sort explicitly if a particular driver order is needed.

Stage metrics use the processed feed's `stage_points` column. Pit-stop rows
returned by `DriversData.driver_pit_stops()` include `race_id`, allowing the
optional race filter to work across a season.

Lap-speed maxima and speed ranks are calculated once per race during a season
build, then shared across drivers. The existing `leader_laps` metric counts laps
with the fastest speed (including ties), rather than laps led by running
position. This optimization preserves that definition and does not cache derived
metrics on mutable `Race` objects.

## Development

Install the package and pytest, then run the offline regression suite:

```bash
python -m pip install -e . 'pytest>=7'
python -m pytest
```

A reproducible lap-analysis benchmark checks exact metric parity against the
original per-driver calculation before timing either implementation:

```bash
PYTHONPATH=src python benchmarks/driver_lap_analysis.py
# Or use an existing Race.telemetry.lap_times parquet file:
PYTHONPATH=src python benchmarks/driver_lap_analysis.py --laps /path/to/laps.parquet
```

The benchmark measures lap analysis for all drivers in one race; it excludes
network/cache I/O and other season aggregation work.
